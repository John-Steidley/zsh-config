...
alias merge_base_main="git merge-base origin/main HEAD"
alias gdm="git diff \$(merge_base_main)"
alias delta="gdm | rg -v -e \"^\\+\\+\\+\" -e "^---" | rg \"^([\\+-]).*\" -r '\$1' | sort | uniq -c"
alias gdb="git diff \$(merge_base_branch)"
alias gdmn="gdm --name-only | cat"
alias gpu="git push -u origin \$(current_branch)"
alias squash="git rebase -i \$(merge_base_main)"
alias squashb="git rebase -i \$(merge_base_branch)"
